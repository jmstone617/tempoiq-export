...
raise ArgumentError.new('TempoIQ API key is missing') unless ENV['TIQ_API_KEY']
raise ArgumentError.new('TempoIQ API secret is missing') unless ENV['TIQ_API_SECRET']
raise ArgumentError.new('TempoIQ host is missing') unless ENV['TIQ_HOST']

tiq = TempoIQ::Client.new(ENV['TIQ_API_KEY'], ENV['TIQ_API_SECRET'], ENV['TIQ_HOST'])

if device_key
  device = tiq.get_device(device_key)
  if device
    ...
    puts "Unable to find device with key #{device_key}"
  end
else
